...
class Embed_Att(nn.Module):
    def __init__(self, opt):
        super(Embed_Att, self).__init__()
        self.fc1 = nn.Linear(opt.attSize, 256)
        #self.fc2 = nn.Linear(opt.ndh, opt.ndh)
        self.fc2 = nn.Linear(256, opt.outzSize)
        self.lrelu = nn.LeakyReLU(0.2, True)
        self.apply(weights_init)
...
class E_MAP(nn.Module):
    def __init__(self, opt):
        super(E_MAP, self).__init__()
        self.fc1 = nn.Linear(opt.outzSize, opt.attSize)
        #self.fc2 = nn.Linear(opt.ndh, opt.ndh)
        #self.fc2 = nn.Linear(opt.nhF, 1)
        self.lrelu = nn.LeakyReLU(0.2, True)
        self.apply(weights_init)
...
class M_Embed_Att(nn.Module):
    def __init__(self, opt):
        super(M_Embed_Att, self).__init__()
        self.fc1 = nn.Linear(opt.outzSize, 256)
        #self.fc2 = nn.Linear(opt.ndh, opt.ndh)
